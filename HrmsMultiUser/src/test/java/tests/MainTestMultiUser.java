...
public class MainTestMultiUser extends BaseTest {
	public static void main_Test() throws Exception {

		// String[] data =
		// Excel_Reader.readData("C:\\\\Users\\\\aadrika\\\\Desktop\\\\HRMS\\\\Onb_Test_1.xlsx");
		// String[] data =
		// Excel_Reader.readData("C:\\Users\\aadrika\\Desktop\\HRMS\\Onboard_data1.xlsx");
		List<String[]> data = ExcelReader_MultiRow
				.getExcelData("C:\\Users\\aadrika\\Desktop\\HRMS\\Onboard_data1.xlsx");
		...
		// Step-2
		/*
		 * Emp_Details ep2 = new Emp_Details(driver); ep2.emp_details(driver, data);
		 * wt.waitTime(); ct.click_button(); wt.waitTime();
		 * 
		 * // Step-3 Emp_Prsnl_Details ep3 = new Emp_Prsnl_Details(driver);
		 * ep3.emp_prsnl_details(driver, data); wt.waitTime(); ct.click_button();
		 * wt.waitTime();
		 * 
		 * // Step-4 Emp_Prnt_Add ep4 = new Emp_Prnt_Add(driver);
		 * ep4.emp_prnt_add(driver, data); wt.waitTime(); ct.click_button();
		 * wt.waitTime();
		 * 
		 * Emp_Edu_Train_Details ep5 = new Emp_Edu_Train_Details(driver);
		 * ep5.emp_edu_train_details(driver, data); wt.waitTime(); ct.click_button();
		 * wt.waitTime();
		 * 
		 * // Step-6 Emp_Join_Inform ep6 = new Emp_Join_Inform(driver);
		 * ep6.emp_join_inform(driver, data); wt.waitTime(); ct.click_button();
		 * wt.waitTime();
		 * 
		 * // Step-7 Emp_Service_History ep7 = new Emp_Service_History(driver);
		 * ep7.emp_service_history(driver, data); wt.waitTime(); ct.click_button();
		 * wt.waitTime();
		 * 
		 * // Step-8 Emp_Family_Details ep8 = new Emp_Family_Details(driver);
		 * ep8.emp_Family_Details(driver, data); wt.waitTime(); ct.click_button();
		 * wt.waitTime();
		 * 
		 * // Step-9 Emp_Sal_Info ep9 = new Emp_Sal_Info(driver);
		 * ep9.emp_sal_info(driver, data); wt.waitTime(); ct.click_button();
		 * wt.waitTime();
		 * 
		 * // Step-10 Emp_Time_Bound_Details ep10 = new Emp_Time_Bound_Details(driver);
