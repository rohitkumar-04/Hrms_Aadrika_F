...
public class Emp_Join_Inform {
	...
	public void emp_join_inform(WebDriver driver, List<String[]> data) throws Exception {
		// Step-6
		...
		segd.selectByVisibleText(data.get(idx)[0]);
		// sed.selectByIndex(1);
		Thread.sleep(3000);
		...
		segde.selectByVisibleText(data.get(idx)[1]);
		// sed.selectByIndex(1);
		Thread.sleep(3000);
		...
		segc.selectByVisibleText(data.get(idx)[2]);
		// sed.selectByIndex(1);
		Thread.sleep(3000);
		...
		if ((data.get(idx)[3]).equalsIgnoreCase("Yes")) {
			System.out.println("Whether Confirmation date = " + data.get(idx)[3]);
			driver.findElement(
